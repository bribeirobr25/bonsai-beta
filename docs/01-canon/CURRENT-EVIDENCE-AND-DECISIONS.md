...
|---|---|---|
| "Bonsai tracking is an empty space" | `CONTRADICTED` | At least 35 distinct bonsai-specific care/tracking/journal products are known to ship or have shipped; exact universe remains `UNVALIDATED` |
...
**Current W1 registry state:** at least **35 distinct bonsai-specific care/tracking/journal products are known to ship or have shipped**. This is an `OBSERVED` discovered lower bound, not a census. The exact universe and current active count remain `UNVALIDATED`.
